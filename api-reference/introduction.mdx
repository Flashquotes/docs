...
import { ProBadge, ScaleBadge } from "/snippets/badge.mdx";
...
Flashquotes exposes two API surfaces, gated by plan:

- **Full REST API** <ScaleBadge /> — every endpoint documented in this reference (everything under `/api/*`) is available on the **Scale** plan. This includes events, bookings, form responses, invoices, and tasks.
- **Zapier endpoints** <ProBadge /> — the Zapier-specific endpoints under `/api/zapier/*` (used by the Flashquotes Zapier app) are available on the **Pro** plan and above. See the [Zapier integration](/settings/integrations/zapier) page for setup.

You can upgrade your plan in [Plan Settings](https://app.flashquotes.com/settings/plans).
...
<Note>
	The API key shown in [Settings > Integrations](https://app.flashquotes.com/settings/integrations) is the **same key** used for both the full REST API and the Zapier endpoints. What it can call depends on your plan: Pro keys can call the Zapier endpoints; Scale keys can call the Zapier endpoints **and** the full REST API. Keep your API key secure and never share it publicly.
</Note>
