...
## Delete Form Pages Instead of Hiding Them

The form builder no longer has a manual **Hide page** toggle. Unneeded pages are now removed by deletion, which keeps the editor a true picture of what customers actually see.

- Empty pages show a kebab menu (⋮) with a **Delete page** option
- Pages with questions can't be deleted until you remove or move the questions off them
- The first page can't be deleted
- For pages that should appear only in some situations, use [conditional logic](/forms/conditional-logic) on booking forms to show or hide based on the customer's service or add-on selection

[Lead intake form editor →](/forms/lead-intake) · [Conditional logic →](/forms/conditional-logic)

</Update>

<Update label="August 19, 2026">

## Event Address Required Before Booking

The customer-facing booking form now requires a complete, dispatchable event address before customers can advance past that step. The **Next** button stays disabled with an inline message until the address is complete.

- A complete address needs a street, a locality (city or town), and a state (US, CA, AU) or postcode (UK, IE) based on your company's country
- The address is prefilled from the quote, so most customers just confirm and continue
- Because lead intake still accepts free text (like "Paris" or "France"), leads with partial addresses will now finish the address at booking time instead of shipping incomplete
- Custom Address questions that aren't linked to the built-in event address field are unchanged and still accept free text

[Learn more →](/contacts/booking-flow#event-address-requirement)

---

## Resources API <ScaleBadge />
...
Requires a Scale-plan API key. Follows the same Stripe-style conventions as the rest of the public API (snake_case query params, camelCase response keys, standard list envelope).
...
## Booked and Lost Columns on the Pipeline

See closed-out quotes without leaving the board. Toggle **Show booked and lost** in the Pipeline header (desktop only) to add read-only **Booked** and **Lost** columns to the right of the board.

- Each column is scoped to the past 90 days by when the quote was resolved, independent of the main board's window
- Loads 25 quotes at a time with a **Load *N* more** button, so old outcomes don't slow the board down
- Handy for reviewing recent wins, spotting patterns in lost reasons, or reopening a quote when a lead comes back

[Learn more →](/pipeline#stages)

---

## Redesigned Filters on the Pipeline and Bookings

Filtering both pages now works the same way: click the dashed **Filter** button to add a filter, and each active filter shows as a pill you can click to edit or remove with the **×**. Add as many as you need — filters combine to narrow the view.

- **Pipeline** filters: **Quote date**, **Event date**, **Source**, **Event type**, **Location**, and **Owner**
- **Bookings** filters: **Booking date**, **First event date**, **Location**, and **Payment status**
- **Quote date** and **Event date** on the Pipeline (and **Booking date** and **First event date** on Bookings) are now independent — apply either, both, or neither, instead of swapping between them. For example, filter to quotes created in the last 30 days *and* events happening in the next 3 months at the same time.

[Pipeline filters →](/pipeline#sorting-and-filtering) · [Bookings filters →](/bookings#filtering-bookings)

---

## Event Type Required When Creating a Quote Manually

The **New quote** flow on the Pipeline and the **Create quote** modal on a contact's **Quotes** tab now require you to pick an **event type** up front. Event type drives pricing and quote defaults, so having it set from the start keeps manually created quotes consistent with quotes that come in through your lead intake form.

[Creating quotes →](/contacts/quotes)

---

## Expire a Quote in One Step

Manually expire a quote when a lead goes cold or pricing is no longer good, without hunting through stage menus.

- **Expire now** is available from the quote page's **More actions** menu and from the row menu on a contact's **Quotes** tab (hidden on quotes that are already expired)
- **Confirmation dialog** shows the contact, event date, and current expiration so you can double-check you're on the right quote before confirming
- **Also mark this quote as Lost** checkbox lets you expire and mark lost in the same step, with the usual reason chips (Price, Date unavailable, Went silent, Chose someone else, Other)
- Reasons are optional, and you can restore the quote from Lost later if the lead re-engages

[Learn more →](/contacts/sending-quotes#expiring-a-quote-manually)

---

## Retrieve a Staff Member via API <ScaleBadge />
...
<Frame>
  <img
    ...
    alt="The new Pipeline board showing quote cards organized into stages"
  />
</Frame>
...
- **Custom stages** <PlusBadge /> — add, rename, and reorder stages to match your sales process
...
<Frame>
  <img
    src="../images/forms/theme-design.gif"
    alt="Customizing the lead intake form theme in the form editor"
  />
</Frame>
...
<Frame>
  <img
    src="../images/changelog/cta-button-edit.gif"
    alt="Editing the CTA button label inline in the lead intake form editor"
  />
</Frame>
...
## Service-Level Travel Fees <PlusBadge />
...
Additionally, you can now export both Invoice and Payments to CSV to support accounting and data analysis.
...
## Multiple Quote Follow-Up Workflows <PlusBadge />
...
## Service-Specific Add-ons <PlusBadge />
...
## Service-Specific Resources <PlusBadge />
...
## Conditional Logic for Booking Forms <PlusBadge />
...
## QuickBooks Deposit Sync Improvements <ProBadge />
...
## Google Analytics & Tag Manager Integration <ProBadge />

<Frame>
  <img
    src="/images/changelog/google-analytics-gtm.webp"
    alt="Google Tag Manager and Google Analytics integration with Flashquotes"
  />
</Frame>
...
- Set a default payment terms to apply to all new quotes
...
## First Event Start Trigger <PlusBadge />
...
<Frame>
  <img
    ...
    alt="Demonstration of changing quote expiration dates from the contact details page"
  />
</Frame>
...
<Frame>
  <img
    ...
    alt="Quote template customer experience showing modern e-commerce inspired design"
    height="300"
  />
</Frame>
