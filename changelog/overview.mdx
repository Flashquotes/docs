...
You can now restrict which add-ons are available for each service. When a customer selects services on a quote request form, only the add-ons linked to those services are shown. If no add-ons are linked, all add-ons remain available.

- **Configure per service** — On the service detail page, use the new "Available Add-ons" section to select which add-ons apply
- **Multiple services** — When multiple services are selected, the customer sees every add-on allowed by any of the selected services
...
You can now restrict which resources are eligible for each service, ensuring the right equipment is matched to the right services.

- **Configure per service** — On the service detail page, use the new "Available Resources" section to select which resources can fulfill the service
- **Date selection** — The date picker on your lead intake form now reflects only the resources eligible for the selected services
- **Availability badges** — Quote presentation and booking form availability badges update based on eligible resources
- **Multi-day events** — The date picker checks that eligible resources are available for every consecutive day, not just the start date
- **Multiple services** — When a quote includes multiple restricted services, only resources eligible for all services count toward availability
...
[Learn about travel fee settings →](/settings/quotes#travel-fee-calculation)
