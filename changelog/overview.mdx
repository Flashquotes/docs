...
## Find Event Inquiries in Your Gmail

Connect your Google inbox and Flashquotes scans your recent email for event inquiries, then surfaces what it finds as suggestion cards in the Pipeline's **New inquiry** column — so leads that arrived by email don't slip through.

- **Scan runs in the background** — a progress banner keeps you posted after connecting, and a completion banner reports how many inquiries that scan found (or that it found none)
- **Review before anything is created** — each suggestion opens a reviewer with the contact, event details, an AI summary, and the source email thread
- **Build the quote in one click** — promoting a suggestion creates the contact and a draft quote and drops you into the quote editor
- **Discard with a reason** — dismiss suggestions that aren't real inquiries, with an optional note
- **Not just a one-time scan** — new inquiries from incoming email keep appearing while your inbox stays connected

You'll also find a **Connect Google Inbox** task in the home welcome checklist, and the Contacts and Inbox pages now prompt you to connect when no inbox is linked.

[Inquiries found in Gmail →](/pipeline#inquiries-found-in-gmail)
