...
## Full REST API on Scale <ScaleBadge />

The full Flashquotes REST API is now available on the **Scale** plan — read and write events, bookings, form responses, invoices, and tasks straight from your own tools.

- Endpoints for events, bookings, form responses, invoices, and task creation, with cursor-based pagination and `expand[]` for related objects
- Stripe-style conventions: `snake_case` query params, `camelCase` responses, structured error envelope, and date-range filters (`[gte]`, `[lt]`, etc.)
- One API key powers both Zapier and the REST API — generate it from **Settings → Integrations**

[Read the API reference →](/api-reference/introduction)

## Zapier now included on Pro <ProBadge />

The Flashquotes Zapier app is now available on the **Pro** plan (previously higher-tier only), so more teams can automate lead, event, and invoice workflows across thousands of apps.

- Same API key works for Zapier on Pro and for the REST API on Scale
- Triggers for new leads, bookings, status changes, invoices, and payments
...
[Learn about per-line-item discounts →](/leads/quotes#per-line-item-discounts)
