...
The booking form's event address question (linked to the built-in `eventAddress` field) requires a complete, dispatchable address before customers can advance. The **Next** button stays disabled with an inline message until the address includes:

- A street
- A locality (city or town)
- A state (US, CA, AU) or postcode (UK, IE), based on your company's country
