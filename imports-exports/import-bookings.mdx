...
<Steps>
  <Step title="Download the CSV Template">
    Go to [Bookings](https://app.flashquotes.com/bookings) and click **Import from CSV**. Download the template with example rows. The template includes an **Email** column, which is now required for every row.
  </Step>
...
- Entity to Invoice
- Full Name
- Email (must be a valid email address; rows without one fail validation and are not imported)
- Payment Method
- Total Price (Dollars)
