...
2. **Gmail Connection**: A Google account connected to Flashquotes. You can connect one directly from the Inbox (see below), or manage connections from Settings → Integrations.

### Accessing the Inbox

1. Navigate to **Inbox** from the main navigation menu.
2. If no Google account is connected yet, click **Connect Google** in the empty state and complete Google's consent screen. You return to the Inbox with a "Google connected" confirmation banner. The banner auto-dismisses after about 12 seconds, or click **X** to close it.
3. Your synced email threads appear automatically once sync catches up.
4. Use the account filter to switch between connected Gmail accounts.

<Note>
Use Settings → Integrations to reconnect an account, add another Gmail account, or manage existing connections.
</Note>

### Waiting for your first emails

Right after you connect a Google account, the Inbox shows a **Waiting for your first emails** state instead of "All caught up!" for the first few minutes while Gmail's initial backfill runs. Once the first sync completes, matching threads appear automatically.
