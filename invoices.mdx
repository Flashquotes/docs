...
<AccordionGroup>
  ...
  <Accordion icon="file-export" title="CSV Export">
...
<AccordionGroup>
  ...
  <Accordion icon="badge-percent" title="Per-line-item Discounts">
...
### Changing the Invoice Contact

Invoices belong to the booking's contact. To bill a different person, change the contact on the booking — you can do this from the invoice editor, the booking page's `More Actions` menu, or the event's Contacts card. All invoices on the booking move to the new contact.

Automated [invoice reminders](/invoices/reminders) address the booking's current contact at the moment each reminder sends, so upcoming reminders follow the new contact automatically.

<Info>
  See [Changing the booking contact](/bookings#changing-the-booking-contact) for the full list of what moves with the booking.
</Info>
...
4. Re-send the invoice link to the customer using **Send** or the copy-link action, or point them to the payment link on the booking confirmation page.
...
<AccordionGroup>
  ...
  <Accordion icon="calendar" title="Payment Schedule">
    ...
    <Frame>
      <img src="/images/invoices/payment-schedule.png" alt="Payment schedule interface showing how to add installments" />
    </Frame>
