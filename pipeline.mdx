...
### Inquiries found in Gmail

Connect your Google inbox and Flashquotes scans your recent email for event inquiries. Anything it finds appears at the top of the **New inquiry** column as a suggestion card — marked "Inquiry found · ready for review" — so leads that arrived by email don't sit unnoticed in your inbox.

Here's how the flow works:

- **Connect** — if your Google inbox isn't connected yet, a **Find inquiries in your inbox** card in the New inquiry column (and a **Connect Google Inbox** task in the home welcome checklist) starts the connection
- **Scan** — right after connecting, Flashquotes scans recent email in the background; a progress banner keeps you posted, and you can keep working while it runs
- **Review** — when the scan finishes, a banner reports how many inquiries that scan found (or tells you it found none). Click a suggestion card to open the reviewer: contact details, event date, location, guest count, an AI summary, and the source email thread
- **Build quote** — creates the contact and a draft quote from the inquiry and hands you straight into the quote editor
- **Discard** — removes the suggestion with a reason (not a quote request, already quoted, event passed, duplicate, or other) and an optional note

Suggestions aren't quotes yet — they don't count toward the column's totals until you build the quote. And the scan isn't a one-time thing: as long as your inbox stays connected, new inquiries from incoming email keep surfacing in the same spot. See [Full Gmail Integration](/settings/integrations/google-gmail-read) for connection details.
