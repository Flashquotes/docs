...
```
Final Price = MAX(
  Base Price + (Staff × Hours × Hourly Rate) + Guest Component + (Resources × Per Resource Rate),
  Minimum Price
)
```
...
- **Minimum price**: Optional floor for variable-rate add-ons (Per guest, Per hour, Per resource, Per unit per hour, Per unit per guest); if the calculated price is below the minimum, the minimum is used
