...
<Note>
Every service you sell needs a staffing rule. If a selected service has no rule, Flashquotes can't resolve staff counts for it — the [lead form's staffing calculator](/forms/staffing-calculator) shows a deferred message instead of a number, and the quote flags the service as unresolved.
</Note>
...
## How the Lead Form Displays Rule Results

The Staffing Calculator question on a lead intake form reads directly from these rules. It shows the resolved count as prose (for example, *"Two professional baristas"*) and, if no rule resolves for the selected services, hides the counter and shows a deferred message instead. See [Staffing Calculator](/forms/staffing-calculator) for the customer-facing behavior.

## Common Questions

**What if only some of my services have rules?**
Services without a rule can't be priced or staffed automatically. On the lead form, the counter defers to a "we'll confirm when we send your quote" message; on the quote, the service is flagged as unresolved until you configure its rule.
...
Rules can't be removed — every service needs one. If you want a simple flat approach, use Hourly Capacity with a single staff member and no maximum.
