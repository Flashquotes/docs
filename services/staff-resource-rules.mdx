...
The Staffing Calculator question on a lead intake form reads directly from these rules. It attributes the estimate ("Estimated for {service}") and, if no rule resolves for the selected services, hides the counter and shows a deferred message instead. See [Staffing Calculator](/forms/staffing-calculator) for the customer-facing behavior.
