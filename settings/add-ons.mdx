---
title: "Add-Ons"
description: "Configure and manage service add-ons and pricing options"
---

Manage your add-ons from Services & items in the sidebar, not Settings.

## Where to find add-ons

Go to Services & items > Add-ons tab to create and manage additional services for your quotes and bookings. Each add-on can be configured with specific pricing models and automation IDs for integration purposes.

## Creating add-ons

<Steps>
  <Step title="Navigate to add-ons">
    Go to Services & items in the sidebar, then open the Add-ons tab
  </Step>
  <Step title="Add new add-on">
    Click "Create Add-on" to set up:
    - Unique identifier
    - Display name
    - Description
    - Pricing structure
    - Quantity limits (for Per unit, Per unit per hour, and Per unit per guest)
    - Minimum price (optional, for variable-rate add-ons)
    - Availability rules
  </Step>
</Steps>
    
### Quantity limits
For **Per unit**, **Per unit per hour**, and **Per unit per guest** add-ons, set minimum and maximum quantity controls:

- **Min Quantity**: Enforce minimum order requirements
- **Max Quantity**: Prevent over-ordering or resource conflicts
- **Default Quantity**: Uses minimum if set, otherwise defaults to 1

**Examples**:
- Cocktail tables: Min 2, Max 10 (event space limitations)
- Bartenders: Min 1, Max 4 (staffing availability)
- Photo booth props: Min 5, Max 25 (package deals)

<Note>
Quantity limits only apply to Per unit, Per unit per hour, and Per unit per guest. Flat rate and other pricing models don't use these controls.
</Note>

<Note>
Unique IDs are essential for automation workflows and integrations.
</Note>

## Pricing options

Configure pricing models:
- Flat rate pricing
- Per unit pricing
- Per guest pricing
- Per hour pricing
- Per resource pricing
- Per unit per hour pricing
- Per unit per guest pricing (quantity × guest count × days)
- Minimum price (optional) for variable-rate add-ons — ensures the calculated price never goes below that floor

<Tip>
Choose pricing models that align with your operational costs and market positioning.
</Tip>

## Managing add-ons

From the Add-ons tab under Services & items:

**Settings control**:
- Availability status
- Display order
- Category assignment
- Required resources

**Visual presentation**:
- Add-on images
- Description formatting
- Feature highlights
- Upsell messaging

## Integration setup

**Automation details**:
- Automation IDs
- System linkages
- Trigger conditions
- Workflow connections

<Warning>
Maintain consistent IDs across all integrated platforms.
</Warning>

**Pricing rules**:
- Quantity limits for Per unit, Per unit per hour, and Per unit per guest add-ons
- Minimum and maximum order requirements
- Minimum price for variable-rate add-ons (Per guest, Per hour, Per resource, Per unit per hour, Per unit per guest) so the calculated price never goes below that floor
- Bundle discounts
- Special pricing conditions

### How quantity limits work

**Minimum Quantity**:
- Prevents customers from ordering below your business requirements
- Automatically sets as default quantity when customers add the item
- Useful for items that require minimum orders to be profitable

**Maximum Quantity**:
- Prevents over-ordering beyond your capacity
- Helps manage resource allocation and logistics
- Protects against inventory shortages

**Common use cases**:
- Audio equipment: Min 1 speaker, Max 8 speakers (venue size limits)
- Linens: Min 10 napkins, Max 200 napkins (bulk order efficiency)
- Staff: Min 1 server, Max 6 servers (team size management)

## Best Practices

1. Use clear, descriptive names
2. Set appropriate pricing models
3. Maintain accurate descriptions
4. Update availability regularly
5. Test automation workflows

## Next steps

After configuring add-ons:
- [Review quote settings](/settings/quotes)
- [Set up integrations](/settings/integrations)
- Test add-on functionality
- Monitor sales performance 