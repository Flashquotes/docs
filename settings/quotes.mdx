...
<AccordionGroup>
  <Accordion icon="bolt" title="Instant Quoting Settings">
    ...
    When enabled, all instant quotes are automatically enrolled in your [default Quote Sent workflow](/workflows/triggers#quote-sent). Pick the default from the workflow selector next to the toggle. This ensures consistent follow-up for every contact without manual intervention.

    <Warning>
      A default Quote Sent workflow must be set for instant-quote auto-enrollment to work. If you have multiple Quote Sent workflows and no default, instant quotes won't enroll in any of them.
    </Warning>
