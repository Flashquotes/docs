...
description: "Learn how to enroll contacts in workflows and manage active enrollments"
---

import { PlusBadge } from "/snippets/badge.mdx";

## Enrolling Contacts in Workflows <PlusBadge />
...
<Frame>
	<img
		...
		alt="Contact details workflow tab showing active, completed and canceled workflows"
	/>
</Frame>

### Contact Quote List

Within the quotes tab in the contact details view, you can easily identify quotes enrolled in a workflow with orange workflow enrollment icon and counter

<Frame>
	<img
		src="/images/workflows/lead-quote-tab.png"
		alt="Contact details quote tab showing quotes with an icon indicating active workflow enrollments"
	/>
</Frame>
...
2. **From Contact Workflows Tab**:
   ...
   - You can easily click **Stop Workflow** on all active workflow for this contact
...
Workflows stop automatically in two situations:

**When a contact books a quote**, all active workflows for that contact stop. This includes workflows across multiple quotes. You won't keep sending sales-focused follow-ups after they've decided to work with you.

**When a contact replies to your email**, active Quote Requested and Quote Sent workflows stop. This requires the [full Gmail integration](/settings/integrations/google-gmail-read) on a Plus or Pro plan. Only workflows started before the reply are stopped.

<Warning>
	Without the full Gmail integration, a contact's reply won't stop active
	workflows. Stop them manually from the quote view or the contact's Workflows
	tab.
</Warning>

**Quote expiration doesn't stop a workflow.** The enrollment stays active, but Flashquotes skips scheduled workflow emails while the quote is expired, lost, or deleted. If you [extend the quote's expiration date](/settings/quotes), remaining scheduled emails send as normal.
